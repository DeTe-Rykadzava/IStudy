...
public class OwnerTypeController : BaseController
{
    ...
    [Authorize]
    [HttpPost]
    public async Task<ActionResult<CertificateOwnerTypeDTO>> PostCertificateOwnerType(
        CertificateOwnerTypeDTO certificateOwnerType)
    {
        if (!await IsAdmin(_context)) return Forbid();
    ...
    public async Task<IActionResult> DeleteCertificateOwnerType(int id)
    {
        ...
        if (certificateOwnerType == null) return NotFound();
